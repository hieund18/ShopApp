...
public class OrderRepositoryCustomImpl implements OrderRepositoryCustom {
    ...
    @Override
    public Page<Order> findOrdersByQuerydsl(String keyword, Float totalMoneyFrom, Float totalMoneyTo, LocalDate startDate,
        ...
        if (keyword != null && !keyword.isEmpty()) {
            builder.and(order.fullName.containsIgnoreCase(keyword)
                    .or(order.phoneNumber.containsIgnoreCase(keyword))
                    .or(order.address.containsIgnoreCase(keyword)));
        }

        if (totalMoneyFrom != null)
            builder.and(order.totalMoney.goe(totalMoneyFrom));

        if (totalMoneyTo != null)
            builder.and(order.totalMoney.loe(totalMoneyTo));

        if (startDate != null)
            builder.and(order.createdAt.goe(startDate.atStartOfDay()));

        if (endDate != null)
            builder.and(order.createdAt.loe(endDate.atStartOfDay()));

        if (status != null && !status.isEmpty())
            builder.and(order.status.eq(status));

        if (isActive != null)
            builder.and(order.isActive.eq(isActive));
        ...
        pageable.getSort().forEach(order1 -> {
            Path<?> path = switch (order1.getProperty()) {
                case "totalMoney" -> order.totalMoney;
                default -> order.createdAt;
            };

            query.orderBy(order1.isAscending()
